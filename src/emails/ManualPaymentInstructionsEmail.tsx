    ...
    <BaseLayout previewText={preview}>
      <Section>
        ...
        {isTransfer && props.bank ? (
          ...
          <Text style={text}>
            Elegiste pago en efectivo. Te vamos a contactar para coordinar el pago o la entrega.
          </Text>
        )}

        {(props.instrucciones || props.bank?.instrucciones) && (
          <Text style={text}>
            <strong>Importante:</strong> {props.instrucciones ?? props.bank?.instrucciones}
          </Text>
        )}
        {!isTransfer && (
          <Text style={textMuted}>
            Incluí el número de pedido ({props.externalOrderId}) al coordinar el pago.
          </Text>
        )}
